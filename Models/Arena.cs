...
internal class Arena
{
    ...
    public void OnPlayerSpawn(CCSPlayerController playerController)
    {
        ...
        if (wasPlayer1 || wasPlayer2)
        {
            ...
            if (isP1Valid())
            {
                ...
                var pawn1 = _player1!.PlayerController.PlayerPawn.Value;
                // Teleport player there
                if (pos != null && angle != null && pawn1 != null)
                {
                    pawn1.Teleport(pos, angle, velocity);
                }

                // Reset weapons and health
                _player1.ResetPlayerWeapons(_roundType);
                if (pawn1 != null) pawn1.Health = 100;
            }
            ...
            if (isP2Valid())
            {
                ...
                var pawn2 = _player2!.PlayerController.PlayerPawn.Value;
                // Teleport player there
                if (pos != null && angle != null && pawn2 != null)
                {
                    pawn2.Teleport(pos, angle, velocity);
                }
                _player2.ResetPlayerWeapons(_roundType);
                if (pawn2 != null) pawn2.Health = 100;
            }
        }
    }
    ...
    private bool isP1Valid()
    {
        return _player1 != null 
            && _player1.PlayerController.IsValid 
            && _player1.PlayerController.Connected == PlayerConnectedState.PlayerConnected 
            && _player1.PlayerController.PlayerPawn.IsValid;
    }
