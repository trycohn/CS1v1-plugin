...
public class CS2Multi1v1 : BasePlugin
{
    ...
    public HookResult OnPlayerActivate(EventPlayerActivate @event, GameEventInfo info)
    {
        ...
        if (playerController.IsBot) return HookResult.Continue;
    ...
    private void TryAssignWaitingPlayers()
    {
        _logger.LogInformation($"TryAssignWaitingPlayers: Queue={_waitingArenaPlayers.Count}, Arenas={_rankedArenas.Count}");

        if (_rankedArenas.Count == 0)
        {
            _logger.LogWarning("No arenas available!");
            return;
        }

        // Находим свободную арену (где нет обоих игроков)
        while (_waitingArenaPlayers.Count >= 2)
        {
            Arena? freeArena = _rankedArenas.FirstOrDefault(a => a._player1 == null && a._player2 == null);
            if (freeArena == null)
            {
                // Ищем арену с одним игроком
                freeArena = _rankedArenas.FirstOrDefault(a => a._player1 == null || a._player2 == null);
            }

            if (freeArena == null)
            {
                _logger.LogInformation("No free arenas available");
                break;
            }

            ArenaPlayer player1 = _waitingArenaPlayers.Dequeue();
            ArenaPlayer player2 = _waitingArenaPlayers.Dequeue();

            int arenaIndex = _rankedArenas.IndexOf(freeArena);
            freeArena.AddPlayers(player1, player2, arenaIndex + 1);
            _logger.LogInformation($"Assigned {player1.PlayerController.PlayerName} and {player2.PlayerController.PlayerName} to arena {arenaIndex + 1}");
        }

        // Если остался один игрок и есть полностью свободная арена - добавляем его одного
        if (_waitingArenaPlayers.Count == 1)
        {
            Arena? freeArena = _rankedArenas.FirstOrDefault(a => a._player1 == null && a._player2 == null);
            if (freeArena != null)
            {
                ArenaPlayer player1 = _waitingArenaPlayers.Dequeue();
                int arenaIndex = _rankedArenas.IndexOf(freeArena);
                freeArena.AddPlayers(player1, null, arenaIndex + 1);
                _logger.LogInformation($"Assigned {player1.PlayerController.PlayerName} alone to arena {arenaIndex + 1}");
            }
        }
    }
    ...
    public void SetupArenasIfNeeded()
    {
        if (!_aimMapLoaded)
        {
            ...
            _logger.LogInformation($"SetupArenasIfNeeded: Created {_rankedArenas.Count} arenas on map {Server.MapName}");
            _aimMapLoaded = true;
        }
    }
